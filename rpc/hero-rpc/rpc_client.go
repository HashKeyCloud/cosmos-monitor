...
func InitHeroRpcCli() (*HeroCli, error) {
	endpoint := fmt.Sprintf("%s:%s", viper.GetString("gRpc.heroIp"), viper.GetString("gRpc.heroPort"))
	grpcConn, err := rpc.InitChainRpcCli(endpoint)
	if err != nil {
		logger.Error("Failed to create hero gRPC client, err:", err)
		return nil, err
	}
