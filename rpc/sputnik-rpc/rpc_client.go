...
type SputnikCli struct {
	*rpc.ChainCli
}

// InitRpcCli Create a new RPC service
func InitSputnikRpcCli() (*SputnikCli, error) {
	endpoint := fmt.Sprintf("%s:%s", viper.GetString("gRpc.sputnikIp"), viper.GetString("gRpc.sputnikPort"))
	grpcConn, err := rpc.InitChainRpcCli(endpoint)
	if err != nil {
		logger.Error("Failed to create sputnik gRPC client, err:", err)
		return nil, err
	}
