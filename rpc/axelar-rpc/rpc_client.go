...
type AxelarCli struct {
	*rpc.ChainCli
}

// InitRpcCli Create a new RPC service
func InitAxelarRpcCli() (*AxelarCli, error) {
	endpoint := fmt.Sprintf("%s:%s", viper.GetString("gRpc.axelarIp"), viper.GetString("gRpc.axelarPort"))
	grpcConn, err := rpc.InitChainRpcCli(endpoint)
	if err != nil {
		logger.Error("Failed to create axelar gRPC client, err:", err)
		return nil, err
	}
