...
type GopherCli struct {
	*rpc.ChainCli
}

// InitRpcCli Create a new RPC service
func InitGopherRpcCli() (*GopherCli, error) {
	endpoint := fmt.Sprintf("%s:%s", viper.GetString("gRpc.gopherIp"), viper.GetString("gRpc.gopherPort"))
	grpcConn, err := rpc.InitChainRpcCli(endpoint)
	if err != nil {
		logger.Error("Failed to create gopher gRPC client, err:", err)
		return nil, err
	}
