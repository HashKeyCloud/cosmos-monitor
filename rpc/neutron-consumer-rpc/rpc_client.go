...
type NeutronconsumerCli struct {
	*rpc.ChainCli
}

// InitRpcCli Create a new RPC service
func InitNeutronconsumerRpcCli() (*NeutronconsumerCli, error) {
	endpoint := fmt.Sprintf("%s:%s", viper.GetString("gRpc.neutronconsumerIp"), viper.GetString("gRpc.neutronconsumerPort"))
	grpcConn, err := rpc.InitChainRpcCli(endpoint)
	if err != nil {
		logger.Error("Failed to create neutron consumer chain gRPC client, err:", err)
		return nil, err
	}
