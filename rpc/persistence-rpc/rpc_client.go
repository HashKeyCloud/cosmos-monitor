...
type PersistenceCli struct {
	*rpc.ChainCli
}

// InitRpcCli Create a new RPC service
func InitPersistenceRpcCli() (*PersistenceCli, error) {
	endpoint := fmt.Sprintf("%s:%s", viper.GetString("gRpc.persistenceIp"), viper.GetString("gRpc.persistencePort"))
